...
class JSONDataset(Dataset):
    ...
    def __getitem__(self, idx):
        with open(self.files[idx], 'r') as f:
            data = json.load(f)
        acceleration = data.get('velocity', 0.0)
        return {
            'velocity': torch.tensor(acceleration, dtype=torch.float32)
        }
...
def plot_true_acceleration(dataloader_val, save_folder=None):
    ...
    for data in dataloader_val:
        accelerations = data['velocity'].detach().cpu().numpy()
    ...
    plt.plot(true_accelerations, label='velocity', color='blue')
    ...
    if save_folder:
        ...
        save_path = os.path.join(save_folder, f"velocity_plot_{timestamp}.png")
