...
filtered_indices = (x >= x_range[0]) & (x <= x_range[1]) & (y >= y_range[1]) & (y <= y_range[0])
...
# **zx方向**の視点に設定
ax.view_init(elev=0, azim=90)  # y方向から見た視点

# 保存
output_file_path_zx = os.path.join(output_folder, "3d_scatter_plot_carla_zx_view.png")
plt.savefig(output_file_path_zx)
print(f"プロット結果 (zx方向) を保存しました: {output_file_path_zx}")
