...
def count_accelerating_segments_with_overlap(dataset_path, output_folder, threshold=1.0, frame_window=10):
    ...
    for root, _, files in os.walk(dataset_path):
        ...
        # Process each set of files with overlapping windows
        accelerations = []
        for i, file in enumerate(json_files):
            ...
            if len(accelerations) == frame_window:
                ...
                if acceleration_sum >= threshold:
                    ...
                    start_frame = i - frame_window + 1
                    end_frame = i
                    accelerating_segments.append((start_frame, end_frame))

                # Remove the oldest frame to slide the window
                accelerations.pop(0)

        # Store the last segment frames
        if accelerating_segments:
            last_segment_frames = accelerating_segments[-1]

    # Save results to a text file in the parent folder
    output_file = os.path.join(output_folder, "accelerating_segments_positive.txt")
    os.makedirs(output_folder, exist_ok=True)
    with open(output_file, "w") as f:
        # Write the total count
        f.write(f"Number of accelerating segments: {len(accelerating_segments)}\n\n")
        # Write details of each segment
        for segment in accelerating_segments:
            f.write(f"Start Frame: {segment[0]}, End Frame: {segment[1]}\n")
        # Write the last segment frame
        if last_segment_frames:
            f.write(f"\nLast Segment: Start Frame: {last_segment_frames[0]}, End Frame: {last_segment_frames[1]}\n")

    return len(accelerating_segments), output_file, last_segment_frames


if __name__ == "__main__":
    # Argument parser for command line arguments
    parser = argparse.ArgumentParser(description="Count overlapping accelerating segments in dataset for multiple parent folders.")
    ...
    for parent_folder in parent_folders:
        ...
        # Count overlapping accelerating segments and save results
        accelerating_count, output_file, last_segment = count_accelerating_segments_with_overlap(
        ...
        summary_results.append(f"Folder: {os.path.basename(parent_folder)}, "
                               f"Number of accelerating segments: {accelerating_count}, "
                               f"Results saved to: {output_file}, "
                               f"Last Segment: Start Frame: {last_segment[0] if last_segment else 'N/A'}, "
                               f"End Frame: {last_segment[1] if last_segment else 'N/A'}")
        print(f"Processed {parent_folder}: Number of accelerating segments: {accelerating_count}")
        print(f"Results saved to: {output_file}")

    # Save summary results to a single file in the root folder
    summary_file = os.path.join(args.output_folder, "summary_results_positive.txt")
    with open(summary_file, "w") as f:
        f.write("Summary of all results:\n\n")
        for line in summary_results:
            f.write(line + "\n")
        f.write(f"\nTotal Number of Accelerating Segments: {total_accelerating_segments}\n")

    print(f"Summary of results saved to: {summary_file}")
    print(f"Total Number of Accelerating Segments: {total_accelerating_segments}")
