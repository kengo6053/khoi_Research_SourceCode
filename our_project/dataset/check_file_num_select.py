...
def count_files_in_subfolders(root_folder, folder_list_file, output_file):
    """
    ...
    指定されたフォルダリストのみを対象に処理を行う。
    また、結果を指定したファイルに保存する。
    """
    # 指定されたフォルダリストを読み込み
    with open(folder_list_file, 'r') as f:
        specified_folders = set(line.strip() for line in f if line.strip())

    # 各親フォルダを処理
    subdirectories = [os.path.join(root_folder, d) for d in os.listdir(root_folder) if os.path.isdir(os.path.join(root_folder, d)) and d in specified_folders]

    # カウント対象のフォルダ名
    target_folders = ["gray", "measurements", "point_cloud", "point_cloud_transformed"]
...
if __name__ == "__main__":
    # コマンドライン引数でルートフォルダ、フォルダリストファイル、出力ファイルを指定
    ...
    parser.add_argument('--folder_list_file', required=True, help='Path to the text file containing the list of folders to process.')
    parser.add_argument('--output_file', default="${WORK_DIR}/our_project/dataset/result_file_num.txt", help='Path to the output file (default: output.txt).')
    args = parser.parse_args()

    count_files_in_subfolders(args.root_folder, args.folder_list_file, args.output_file)
