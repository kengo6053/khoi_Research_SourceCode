...
def plot_true_acceleration(dataloader_val, save_folder=None):
    ...
    """
    inverted_accelerations = []

    # データローダから反転後の加速度を収集
    for data in dataloader_val:
        accelerations = data['acceleration'].detach().cpu().numpy()
        inverted_accelerations.append(accelerations)

    # 真値を1次元に結合
    inverted_accelerations = np.concatenate(inverted_accelerations, axis=0)

    # グラフをプロット
    plt.figure(figsize=(10, 6))
    plt.plot(inverted_accelerations, label='Inverted Acceleration', color='blue')
    plt.title('Inverted Acceleration Over Frames')
    plt.xlabel('Frame')
    plt.ylabel('Acceleration (Inverted)')
    plt.legend()

    if save_folder:
        os.makedirs(save_folder, exist_ok=True)
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        save_path = os.path.join(save_folder, f"reverse_acceleration_plot_{timestamp}.png")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Plot inverted acceleration")
