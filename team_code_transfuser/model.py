...
class LidarCenterNet(nn.Module):
    ...
    def forward_ego(self, rgb, lidar_bev, target_point, target_point_image, ego_vel, bev_points=None, cam_points=None, save_path=None, expert_waypoints=None,
        ...
        # 5次元の場合、角度方向の次元をバッチ方向に統合
        #if rgb.dim() == 5:
        #    batch_size, aug_degrees, channels, height, width = rgb.size()
        #    rgb = rgb.view(batch_size * aug_degrees, channels, height, width)
        
        # `lidar_bev`の形状確認と修正
        #if lidar_bev.dim() == 5:
        #    # 5次元の場合、チャネル方向を正しく扱えるように変換
        #    batch_size, _, channels, height, width = lidar_bev.size()
        #    lidar_bev = lidar_bev.view(batch_size * _ , channels, height, width)
