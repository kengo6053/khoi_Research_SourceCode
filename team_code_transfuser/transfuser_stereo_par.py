...
class TransfuserBackbone(nn.Module):
    ...
    def __init__(self, config, image_architecture='resnet34', lidar_architecture='resnet18', use_velocity=1):
        ...
        self.lidar_encoder = LidarEncoder(architecture=lidar_architecture, in_channels=2,
                                          out_features=self.config.perception_output_features)
...
class GPT(nn.Module):
    ...
    def __init__(self, n_embd, n_head, block_exp, n_layer, 
        ...
        #print(f"img_vert_anchors: {self.img_vert_anchors}, img_horz_anchors: {self.img_horz_anchors}")
        #print(f"lidar_vert_anchors: {self.lidar_vert_anchors}, lidar_horz_anchors: {self.lidar_horz_anchors}")
...
class ImageCNN(nn.Module):
    ...
    def __init__(self, architecture, normalize=True, out_features=512):
        ...
        if architecture.startswith('regnet'):
            # RegNetの場合、最初の層はself.features.stem.convに存在
...
def normalize_grayscale(x):
    """ Normalize grayscale images to [0, 1] or a custom range.
    Args:
        x (tensor): input images
    """
    x = x.clone()
    return (x / 255.0)  # Normalize to range [0, 1]
...
class LidarEncoder(nn.Module):
    ...
    def __init__(self, architecture, in_channels=2, out_features=512):
        ...
        self._model.conv1 = nn.Conv2d(
            in_channels=in_channels,  # Set to 2 for LiDAR input
            out_channels=_tmp.out_channels,
            kernel_size=_tmp.kernel_size,
            stride=_tmp.stride,
            padding=_tmp.padding,
