...
class StereoData(Dataset):
        ...
        for f in raw_files:
            ...
            if os.path.isfile(img_path) and os.path.isfile(pc_path) and os.path.isfile(meas_path):
                ...
                with open(meas_path, 'r') as mf:
                    ...
                    self.true_accelerations.append(measurement.get("acceleration", 0.0))
            else:
    ...
    def __getitem__(self, idx):
        ...
        true_acceleration = torch.tensor(self.true_accelerations[idx], dtype=torch.float32).unsqueeze(0)
    ...
    for subdir in os.listdir(dataset_path):
        ...
        with torch.no_grad():
            for idx, batch in enumerate(dataloader):
                ...
                true_acceleration = batch["true_acceleration"].item()
                ...
                true_accelerations_list.append(true_acceleration)
        ...
        # MSE 計算
        mse = mean_squared_error(true_accelerations_list, predictions_list)
        ...
        with open(results_file, "w") as out_file:
            json.dump({
                ...
                "true_values": true_accelerations_list,
                "MSE": mse
            }, out_file, indent=2)
        ...
        plt.plot(true_accelerations_list, label="True Acceleration", color="blue")
        ...
        all_values = np.concatenate([true_accelerations_list, predictions_list])
