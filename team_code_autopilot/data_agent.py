...
class DataAgent(AutoPilot):
    ...
    def save_sensors(self, tick_data):
        ...
        # Lidarデータの型を確認して保存
        try:
            lidar_data = tick_data['lidar']
            
            # Lidarデータの形式を確認
            print("Lidar data type:", type(lidar_data))
            print("Lidar data shape:", np.shape(lidar_data))

            # Lidarデータが不均一な場合、dtype=objectで保存する
            if isinstance(lidar_data, list):
                lidar_data = np.array(lidar_data, dtype=object)

            # Lidarデータを保存
            np.save(self.save_path / 'lidar' / ('%04d.npy' % frame), lidar_data, allow_pickle=True)
            print(f"Saved lidar data for frame {frame}")
            
        except Exception as e:
            print(f"Failed to save lidar data for frame {frame}: {e}")
            print(f"Lidar data:", tick_data['lidar'])
